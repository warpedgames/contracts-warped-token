...
describe('StarlTreasuryHandler', function () {
    ...
    beforeEach(async function() {
        ...
        if (this.currentTest.title.includes("after init,")) {
            await this.treasuryHandler.initialize(this.signers[1].address, this.token.address);
        }
        if (this.currentTest.title.includes("after pool added,")) {
            const router = await ethers.getContractAt(uniswapRouterAbi, "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D");
            const factory = await ethers.getContractAt(uniswapFactoryAbi, await router.factory());
    ...
    it("after init, init reverts as already init", async function() {
        await expectRevert(
            this.treasuryHandler.initialize(this.signers[0].address, this.token.address),
            'Already initialized'
        );
    });
    ...
    it("after init, setPriceImpactBasisPoints update correctly", async function() {
        ...
        expect(await this.treasuryHandler.priceImpactBasisPoints()).to.equal(newPoint);
    });
    ...
    it("after init, withdraw successfully withdraw ethers in contract", async function() {
        ...
        await expect(() => this.treasuryHandler.withdraw(constants.ZERO_ADDRESS, ethBalance))
    ...
    it("after init, withdraw successfully withdraw tokens in contract", async function() {
        ...
        await expect(() => this.treasuryHandler.withdraw(this.token.address, tokenAmount))
    ...
    it("processTreasury do nothing if called before init or from is zero or not sell", async function() {
        ...
        await this.token.transfer(this.treasuryHandler.address, tokenAmount);
        await expect(() => this.token.testTreausryHandler(this.treasuryHandler.address, this.signers[0].address, this.signers[1].address, BN.from(100)))
            .to.changeTokenBalance(this.token, this.treasuryHandler, BN.from(0));

        await expect(() => this.token.testTreausryHandler(this.treasuryHandler.address, constants.ZERO_ADDRESS, this.signers[1].address, BN.from(100)))
            .to.changeTokenBalance(this.token, this.treasuryHandler, BN.from(0));
    ...
    it("after init, do nothing if 'to' address is not pool address or from is zero", async function() {
        const tokenAmount = ethers.utils.parseEther("100000");
        await this.token.transfer(this.treasuryHandler.address, tokenAmount);
    ...
    it("after pool added, after init, processTreasury do nothing if balance is less than tax swap", async function() {
        ...
        await expect(() => this.token.testTreausryHandler(this.treasuryHandler.address, this.signers[1].address, primaryPool, BN.from(100)))
            .to.changeTokenBalance(this.token, this.treasuryHandler, BN.from(0));
    });

    it("after pool added, after init, processTreasury work correctly(balance:11k, amount: 1k)", async function() {
    ...
    it("after pool added, after init, processTreasury work correctly(balance:55k, amount: 52k, taxswap: 51k)", async function() {
        ...
        const tokenPrices = await this.router.getAmountsOut(ethers.utils.parseEther("50000"), [this.token.address, this.wethAddress]);
        const tokenPrice = tokenPrices[1];
        ...
        await this.token.testTreausryHandler(
            this.treasuryHandler.address, this.signers[0].address, primaryPool, ethers.utils.parseEther("52000")
        );
        const afterBalance = await ethers.provider.getBalance(this.signers[1].address);
        const convert = (val) => parseFloat(ethers.utils.formatEther(val)).toFixed(3);
        // compare only 3 decimals as there are slight change in price after swap inside contract function
